...
{#if vectorDeploymentsQuery.loading && !vectorDeploymentsQuery.ready}
    <LoadingView title={t("VECTOR_LOADING_TITLE")} message={t("VECTOR_LOADING_MESSAGE")} />
{:else if vectorDeploymentsQuery.error}
    <div class="query-error">
        <ErrorView
            title={t("VECTOR_ERROR_TITLE")}
            message={t("VECTOR_ERROR_MESSAGE")}
        ...
        <ui5-button design="Emphasized" onclick={() => vectorDeploymentsQuery.refresh()}>{t("COMMON_RETRY")}</ui5-button>
    </div>
