...
const testListPath = "/api/v1/projects/" + TestProjectID + "/landscapes"

var testLandscapeList = apiclient.LandscapeList{
	Data: []apiclient.Landscape{
		{
...
func executeListCommand(appConfig *cfg.AppConfig, args ...string) error {
	...
	listCmd, err := landscape.NewListCmd(appConfig)
...
var _ = Describe("list command", func() {
	...
	BeforeEach(func() {
		...
		server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			_ = json.NewEncoder(w).Encode(testLandscapeList)
		}))
	...
	It("succeeds on 200 and prints the response", func() {
		...
		Expect(printed).To(Equal(ToJSON(&testLandscapeList)))
	})
	...
	It("returns an error when 200 response has no body", func() {
		...
		Expect(err.Error()).To(ContainSubstring("landscapes response did not contain a body"))
		ExpectRequest(requests, http.MethodGet, testListPath)
	})
	...
	It("returns an error when output formatting fails", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("formatting landscapes failed"))
	})
	...
	It("wraps transport errors", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("listing landscapes failed"))
	})
	...
	It("returns an error on unexpected status code", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring(fmt.Sprintf("listing landscapes returned HTTP %d", http.StatusInternalServerError)))
