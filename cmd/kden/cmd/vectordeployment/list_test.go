...
const testListPath = "/api/v1/projects/" + TestProjectID + "/vectorDeployments"

var testVectorDeploymentList = apiclient.VectorDeploymentList{
	Data: []apiclient.VectorDeployment{
		{
			Id:          "test-vector-deployment-id",
			LandscapeId: "test-landscape-id",
			StageId:     "test-stage-id",
			Status:      "Ready",
			Vector: apiclient.VectorReference{
...
func executeListCommand(appConfig *cfg.AppConfig, args ...string) error {
	...
	listCmd, err := vectordeployment.NewListCmd(appConfig)
...
var _ = Describe("list command", func() {
	...
	BeforeEach(func() {
		...
		server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			_ = json.NewEncoder(w).Encode(testVectorDeploymentList)
		}))
	...
	It("succeeds on 200 and prints the response", func() {
		...
		Expect(printed).To(Equal(ToJSON(&testVectorDeploymentList)))
	})
	...
	It("returns an error when 200 response has no body", func() {
		...
		Expect(err.Error()).To(ContainSubstring("vector deployments response did not contain a body"))
		ExpectRequest(requests, http.MethodGet, testListPath)
	})
	...
	It("returns an error when output formatting fails", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("formatting vector deployments failed"))
	})
	...
	It("wraps transport errors", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("listing vector deployments failed"))
	})
	...
	It("returns an error on unexpected status code", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring(fmt.Sprintf("listing vector deployments returned HTTP %d", http.StatusInternalServerError)))
