...
The server listens on `http://127.0.0.1:8091` by default. Set `KONFIDENCE_MOCK_API_PORT` to use another port. Swagger UI is available at `http://127.0.0.1:8091/docs/`; follow **Start a mock session** there before running authenticated requests. UI tests can select a response scenario with the `konfidence_mock_scenario` cookie.
