...
.PHONY: verify
verify: fmt-check lint-go lint-config verify-ui test-star test-galaxy test-pkg ## Run Go checks/tests and UI verification.
...
.PHONY: test
test: hermit manifests generate fmt-go vet test-star test-galaxy test-pkg test-kden-cli test-api ## Run all Go unit tests.
...
.PHONY: build
build: manifests generate fmt vet build-star build-galaxy build-kden-cli build-api ## Build all binaries and UI assets.
	$(PNPM) ui:build
...
.PHONY: run-star
run-star: manifests-star generate-star fmt-go vet ## Run the star operator from your host.
	go run ./cmd/star/main.go

.PHONY: run-galaxy
run-galaxy: manifests-galaxy generate-galaxy fmt-go vet ## Run the galaxy operator from your host.
...
.PHONY: docker-build
docker-build: docker-build-star docker-build-galaxy docker-build-api docker-build-ui ## Build container images for all services (local use only).
...
.PHONY: docker-push
docker-push: docker-push-star docker-push-galaxy docker-push-api ## Push container images for all services.
