...
latest_release() {
	...
	if [ -z "$tag" ]; then
		# Distinguish a rate-limited API (HTTP 403, common for anonymous curl |
		# sh) from a genuine "no releases" so the caller's error isn't misleading.
		# GitHub caps unauthenticated requests at 60/hour per IP.
		if [ "$(api_status "releases/latest")" = 403 ]; then
			die "GitHub API rate limit reached (anonymous requests are capped at 60/hour). Set GITHUB_TOKEN, or pin a release with KDEN_VERSION." #lll
		fi
		return 1
	fi
	echo "$tag"
...
# api_status prints the HTTP status code for an API path without downloading the
# body, so latest_release can tell a rate limit apart from a missing release.
api_status() {
	have curl || return 0
	if [ -n "${GITHUB_TOKEN:-}" ]; then
		curl -s -o /dev/null -w "%{http_code}" \
			-H "Authorization: Bearer ${GITHUB_TOKEN}" "${API}/$1" 2>/dev/null || true
	else
		curl -s -o /dev/null -w "%{http_code}" "${API}/$1" 2>/dev/null || true
	fi
}
...
install_from_release() {
	...
	[ -n "$tag" ] || die "could not resolve a release to install: the repository may have no published releases, or the GitHub API was unreachable. Pin one with KDEN_VERSION, or build from source with KDEN_GIT_REF." #lll
...
install_from_source() {
	...
	ref=$KDEN_GIT_REF
	# A ref starting with '-' is parsed as a git flag, not a value (git option-parses
	# before positional args). Reject it — one guard covers both git calls below.
	case $ref in
	-*) die "invalid KDEN_GIT_REF: refs cannot start with '-' ($ref)" ;;
	esac
