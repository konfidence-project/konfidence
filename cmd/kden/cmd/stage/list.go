...
func NewListCmd(appConfig *cfg.AppConfig) (*cobra.Command, error) {
	return &cobra.Command{
		Use:   "list",
		Short: "List stages for a given project id",
		Long:  `Retrieve and display all stages for a given project id.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			projectId, _ := cmd.Flags().GetString("projectId")
			landscapeId, _ := cmd.Flags().GetString("landscapeId")

			params := &apiclient.ListStagesV1Params{}
			if landscapeId != "" {
			...
			response, err := auth.RequestWithAuthRetry(cmd.Context(), authClient,
				func(ctx context.Context) (*apiclient.ListStagesV1Response, error) {
					return authClient.KdenApiClient().ListStagesV1WithResponse(ctx, projectId, params)
				})
			if err != nil {
				return fmt.Errorf("listing stages failed: %w", err)
			}
			...
			default:
				return fmt.Errorf(
					"listing stages returned HTTP %d: %s",
					response.StatusCode(),
