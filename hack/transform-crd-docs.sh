...
# ^ trim trailing blank lines so the file ends with exactly one newline. Keeps
#   crd.md byte-stable through the docs repo's Contents-API base64 round-trip,
#   which otherwise appends a newline and makes every cron sync see a phantom diff.
