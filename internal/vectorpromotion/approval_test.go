...
var _ = Describe("Approve", func() {
	...
	It("rejects an empty approver", func() {
		c := newClient(newPromotion())

		Expect(Approve(ctx, c, key, "")).To(MatchError(ErrApproverMissing))
	})

	It("rejects a promotion that has no approval gate", func() {
		gateless := newPromotion()
		gateless.Spec.RequireApproval = false
		c := newClient(gateless)

		Expect(Approve(ctx, c, key, "alice@example.com")).To(MatchError(ErrApprovalNotRequired))
	})
	...
	It("rejects a superseded promotion as locked", func() {
		...
		Expect(Approve(ctx, c, key, "alice@example.com")).To(MatchError(ErrPromotionSuperseded))
	})
