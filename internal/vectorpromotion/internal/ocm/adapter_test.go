...
import (
	...
	"github.com/konfidence-project/konfidence/internal/vectorpromotion/internal/ocm/mocks"
	...
	Describe("Promote", func() {
		Context("when source resolution fails", func() {
			It("returns ErrFetchingSourceFailed", func() {
				...
				Expect(errors.Is(err, promotion.ErrFetchingSourceFailed)).To(BeTrue())
		...
		Context("when source vector verification fails", func() {
			It("returns ErrFetchingSourceFailed", func() {
				...
				Expect(err).To(HaveOccurred())
				Expect(errors.Is(err, promotion.ErrSourceVerificationFailed)).To(BeTrue())
